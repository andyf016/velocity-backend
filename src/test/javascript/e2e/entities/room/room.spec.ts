...
describe('Room e2e test', () => {
  ...
  it('should create and save Rooms', async () => {
    ...
    await promise.all([roomUpdatePage.setNumberInput('5'), roomUpdatePage.facilitySelectLastOption()]);

    expect(await roomUpdatePage.getNumberInput()).to.eq('5', 'Expected number value to be equals to 5');
  ...
  it('should delete last Room', async () => {
    ...
    expect(await roomDeleteDialog.getDialogTitle()).to.eq('Are you sure you want to delete this Room?');
